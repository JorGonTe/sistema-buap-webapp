...
export class MaestrosService {
  ...
  public validarmaestro(data: any, editar: boolean){
    ...
    // se agrega validacion para el cubiculo, solo debe salir que es campoo requerio
    if(!this.validatorService.required(data["cubiculo"])){
      error["cubiculo"] = this.errorService.required;
    }
    //Agregar validaciones adicionales para el campo "area_investigacion"
    if(!this.validatorService.required(data["area_investigacion"])){
      error["area_investigacion"] = this.errorService.required;
    }
    //Agregar validaciones adicionales para el campo "materias_estudio"
    if(!this.validatorService.required(data["mate"])){
      error["mate"] = this.errorService.required;
    }
